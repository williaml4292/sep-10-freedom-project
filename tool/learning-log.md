...
Takeaways:  
Liquid allows you to set values for a variable and reuse that variable, so later on, you can change the variable once and change the content of every post at once, instead of changing it on all posts. In other words, I can set a variable, `author.name` as williaml4292 and use `author.name` everytime I want to show the name of the author. However, if I want to change williaml4292 to William Li, I can just change the value of `author.name` instead of individually changing it on every single post.
